...
function TaskCardTime(props) {
    ...
    return (
        <Box sx={{width: '30%', height: 100, background: props.cardColor, boxShadow: '0px 4px 4px rgba(0, 0, 0, 0.25)', borderRadius: 30}}>
        <Box sx={{width: '50%', height: 80, color: '#FFFFFF'}}>
        <Stack direction="column">
            <Stack direction="row">
                <Box sx={{paddingLeft: 30, height: 35, paddingTop: 7.5, width: '150%'}}>
                <Typography variant="h6" fontFamily="Sora" noWrap="true" style={{fontWeight: 'bold', overflow: 'hidden' }}> {props.taskName} </Typography>
                </Box>
            </Stack>
